...
$sql_02 = "select * FROM $database.$tablename where id=$in{'arid'}";
...
#print "<h2 style=\"margin-bottom:0;\">Update $tablename Record $in{'arid'} </h2>\n";
#print "</div>\n";
...
print "<font size=\"6\">\n";
#print "Update $tablename Record $in{'arid'} \n";
print $sql_02;
print "</font>\n";
print "</td>\n";
print "</tr>\n";
...
print "<form action=\"commit_update.pl\" method=\"post\">\n";
...
#my $tmpval = $row[1];
if ($row[1] eq "text") {
print "    <textarea cols=\"60\" rows=\"5\" name=\"$row[0]\">$ar_rec[$ind]</textarea>\n"; 
} 
elsif ($row[1] eq "longtext"){
print "    <textarea cols=\"60\" rows=\"10\" name=\"$row[0]\">$ar_rec[$ind]</textarea>\n"; 
} 
else {
print "    <input type=\"text\" size=\"50\" name=\"$row[0]\" value=\"$ar_rec[$ind]\">\n";
}
print "    </td>\n";
...
# Make a small table for the submit
print "<table width=\"100%\" style=\"height:100%;\" cellpadding=\"10\" cellspacing=\"0\" border=\"0\">\n";
print "<tr>\n";
